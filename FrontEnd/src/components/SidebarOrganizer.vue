...
const updateHeader = () => {
  ...
  const index = appPages.findIndex((page) => page.url === currentPath && page.url !== 'logout');
  selectedIndex.value = index;
  headerTitle.value = index !== -1 ? appPages[index].title : '';
};
...
const navigateToProfile = () => {
  isUserMenuOpen.value = false;
  router.push('/web/organizer-profile');
};
