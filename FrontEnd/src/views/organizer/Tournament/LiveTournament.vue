<template>
  <div class="tournament-container">
    ...
    <div v-if="showConfirmModal" class="confirm-modal-overlay" @click="cancelAdvance">
      <div class="confirm-modal" @click.stop>
        <div class="confirm-modal-header">
          <h3>Confirmar avance</h3>
          <button class="close-button" @click="cancelAdvance">×</button>
        </div>
        <div class="confirm-modal-content">
          <p>¿Estás seguro de que quieres avanzar a este jugador a la siguiente ronda?</p>
          <div class="confirm-modal-buttons">
            <button class="cancel-button" @click="cancelAdvance">Cancelar</button>
            <button class="confirm-button" @click="confirmAdvance">Confirmar</button>
          </div>
        </div>
      </div>
    </div>
    ...
    if (transformedRounds.length > 0) {
      ...
      if (lastRound.games && lastRound.games.length > 0) {
        ...
        if (finalMatch.player1.winner) {
          ...
          // si no esta cerrado cerrar el torneo
          if (tournamentData.value.active && !tournamentData.value.closed) {
            console.log(tournamentWinner.value)
            closeTournament()
          }
...
// Función para cambiar el estado del torneo a cerrado
const closeTournament = async () => {
  try {
    const token = localStorage.getItem('token');
    await axios.put(`http://localhost:8082/api/tournaments/${route.params.id}/${tournamentWinner.value.id}/close`,
    {},
    {
      headers: { Authorization: `Bearer ${token}` }
    });
    
    await axios.put(`http://localhost:8083/api/matchmaking/${route.params.id}/close`,
    {},
    {
      headers: { Authorization: `Bearer ${token}` }
    });
    ...
    console.error(`Error al cerrar el torneo ${route.params.id}:`, error);
  }
}
...
const confirmPlayerAdvance = (playerId) => {
...
const confirmAdvance = async () => {
  ...
  try {
    const token = localStorage.getItem('token');
    await axios.put(
      `http://localhost:8083/api/matchmaking/advance/${route.params.id}/${selectedPlayerId.value}`,
      null,
      {
        headers: { Authorization: `Bearer ${token}` }
      }
    ...
    console.error('Error al avanzar jugador:', error);
  }
};
